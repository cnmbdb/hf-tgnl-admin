    ...
    while True:  # 永不停止
        try:
            ...
            if os.path.exists(env_file_path):
                env_mtime = os.path.getmtime(env_file_path)
                if env_mtime > env_last_mtime:
                    env_last_mtime = env_mtime
                    logging.info(f"检测到 .env 文件mtime变化: {env_file_path}")
                    try:
                        reload_callback()
                        logging.info("通过mtime轮询重载 .env 成功")
                    except Exception as e:
                        logging.error(f"通过mtime轮询重载 .env 失败: {e}")
