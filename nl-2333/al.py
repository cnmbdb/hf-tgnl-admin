...
# Webhook 配置（可选）
BOT_WEBHOOK_URL = os.getenv('BOT_WEBHOOK_URL', '').strip()  # 例如 https://your.domain.com
BOT_WEBHOOK_PORT = int(os.getenv('BOT_WEBHOOK_PORT', '8443'))
BOT_WEBHOOK_PATH = os.getenv('BOT_WEBHOOK_PATH', '').strip().strip('/')  # 默认使用 TOKEN 作为 url_path
...
energy_pool_api = ''  # 会在 reload_config()/main() 中从 config.txt 或 .env 读取
# 机器人回调地址 - 从config.txt读取，支持热重载（能量池系统回调机器人用）
bot_notify_url = None  # 会在reload_config()中从 config.txt 或 .env 读取
...
def build_public_endpoint(base_url, path):
    if not base_url:
        return ''
...
def get_env_energy_pool_api():
    return sanitize_placeholder(os.getenv('ENERGY_POOL_API_URL', '').strip())


def get_env_bot_notify_url():
    explicit = sanitize_placeholder(os.getenv('BOT_NOTIFY_URL', '').strip())
    if explicit:
        return explicit

    public_base = sanitize_placeholder(os.getenv('BOT_SERVER_PUBLIC_URL', '').strip())
    if public_base:
        return build_public_endpoint(public_base, 'api/recharge-notify')
...
def load_dotenv_direct():
    """直接从 .env 文件读取配置，绕过 os.getenv 的进程限制"""
    global env_config
    new_env = {}
    try:
        # .env 位于 al.py 的上级目录
        ...
        if os.path.exists(env_path):
            ...
            logging.info(f"已从 {env_path} 直接加载环境变量")
        else:
            # 备选路径：当前目录
            env_path = '.env'
            if os.path.exists(env_path):
                with open(env_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if line and not line.startswith('#') and '=' in line:
                            k, v = line.split('=', 1)
                            new_env[k.strip()] = v.strip()
                logging.info(f"已从当前目录 .env 直接加载环境变量")
    except Exception as e:
        logging.error(f"直接加载 .env 失败: {e}")
...
def get_env_first(keys, fallback=''):
    global env_config
    for key in keys:
        # 优先从文件缓存读取（支持热重载）
    ...
    if fallback_value.startswith('${') and fallback_value.endswith('}'):
        return ''
...
def sync_bot_config():
    ...
    try:
        ...
        env_notify_url = get_env_bot_notify_url()
        if env_notify_url:
            notify_url_to_send = env_notify_url
        elif 'bot_notify_url' not in globals() or not bot_notify_url:
...
def reload_config():
    ...
    try:
        ...
        # 更新全局配置变量
        CUSTOMER_SERVICE_ID = get_env_first(['CUSTOMER_SERVICE_ID'], config.get('CUSTOMER_SERVICE_ID', CUSTOMER_SERVICE_ID))
        bot_id = get_env_first(['BOT_ID'], config.get('bot_id', bot_id))
        group_link = get_env_first(['GROUP_LINK'], config.get('group_link', group_link))
        control_address = get_env_first(['CONTROL_ADDRESS'], config.get('control_address', control_address))
        # 优先使用usdt2trx_private_key，如果没有则使用privateKey
        env_private_key = get_env_first(['USDT2TRX_PRIVATE_KEY', 'BOT_PRIVATE_KEY'])
        if env_private_key:
            privateKey = env_private_key
        elif 'usdt2trx_private_key' in config and config['usdt2trx_private_key'].strip():
            privateKey = config['usdt2trx_private_key'].strip()
        else:
            privateKey = config.get('privateKey', privateKey)
        username = get_env_first(['BOT_API_USERNAME'], config.get('username', username))
        password = get_env_first(['BOT_API_PASSWORD'], config.get('password', password))
        ...
        env_energy_pool_api = get_env_energy_pool_api()
        if env_energy_pool_api:
            energy_pool_api = env_energy_pool_api
            logging.info(f"能量池API（.env覆盖）已更新为: {energy_pool_api}")
        elif 'energy_pool_api' not in config or not config['energy_pool_api'].strip():
        ...
        env_notify_url = get_env_bot_notify_url()
        if env_notify_url:
            bot_notify_url = env_notify_url
            logging.info(f"机器人回调地址（.env覆盖）已更新为: {bot_notify_url}")
        elif 'bot_notify_url' in config and config['bot_notify_url'].strip():
        ...
        if env_private_key:
            usdt2trx_private_key = env_private_key
        elif 'usdt2trx_private_key' in config and config['usdt2trx_private_key'].strip():
        ...
        # 更新机器人用户名（从bot_id提取，优先从环境变量）
        bot_id_for_username = get_env_first(['BOT_ID'], config.get('bot_id', bot_id))
        if bot_id_for_username:
            if bot_id_for_username.startswith('https://t.me/'):
                bot_username = bot_id_for_username.replace('https://t.me/', '').replace('@', '')
            elif bot_id_for_username.startswith('@'):
                bot_username = bot_id_for_username.replace('@', '')
            else:
                bot_username = bot_id_for_username.replace('@', '')
                    ...
                    if result.get("success"):
                        ...
                        try:
                            ...
                            if balance_resp.status_code == 200:
                                ...
                                api_balance = balance_info.get('当前余额(TRX)', balance_info.get('当前余额 (TRX)', 0))
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if update.message is not None and update.message.chat.type == 'private':
        ...
        try:
            ...
            if not user_data:
                # 用户不存在，自动注册
                tg_username = get_username_from_update(update)
                user = update.effective_user
                user_nickname = f"{user.first_name} {user.last_name}" if user.last_name else (user.first_name or '')
                handle_start_command(chat_id, user_nickname, tg_username)
                logging.info(f"[AUTO_REGISTER] 自动注册用户: chat_id={chat_id}, username={tg_username}, nickname={user_nickname}")
        except Exception as e:
                        ...
                        try:
                            ...
                            if create_resp.status_code == 201:
                                ...
                                if create_data.get('success') and create_data.get('data'):
                                    ...
                                    def _upsert_env_line(lines, key, value):
                                        found = False
                                        updated = []
                                        for line in lines:
                                            stripped = line.strip()
                                    ...
                                    env_lines = _upsert_env_line(env_lines, 'BOT_API_USERNAME', new_username)
                                    env_lines = _upsert_env_line(env_lines, 'BOT_API_PASSWORD', new_password)
...
def webhook_health_check():
    """定期检查 webhook 是否正常，如果发现异常自动重新设置"""
    try:
        # 获取当前 webhook 信息
        ...
        if not webhook_info.url:
            logging.warning("[Webhook健康检查] 未配置 webhook，尝试重新设置...")
            if BOT_WEBHOOK_URL:
                bot.delete_webhook()
                time.sleep(0.5)
                webhook_full_url = build_public_endpoint(BOT_WEBHOOK_URL, BOT_WEBHOOK_PATH or TOKEN)
                bot.set_webhook(webhook_full_url)
                logging.info(f"[Webhook健康检查] 已重新设置 webhook: {webhook_full_url}")
        else:
            # 检查 webhook 是否指向正确的地址
            expected_url = build_public_endpoint(BOT_WEBHOOK_URL, BOT_WEBHOOK_PATH or TOKEN)
            # 仅当 expected_url 有效时才更新（Telegram 不接受无效地址）
            if expected_url and not expected_url.startswith(('0.0.0.0', '127.0.0.1', 'localhost')):
                if webhook_info.url != expected_url:
                    logging.warning(f"[Webhook健康检查] webhook URL 不匹配: 期望={expected_url}, 实际={webhook_info.url}")
                    bot.delete_webhook()
                    time.sleep(0.5)
                    bot.set_webhook(expected_url)
                    logging.info(f"[Webhook健康检查] 已更新 webhook: {expected_url}")
                else:
                    logging.debug(f"[Webhook健康检查] webhook 正常: {webhook_info.url}")
            else:
                logging.debug(f"[Webhook健康检查] webhook URL 无效，跳过检查: {expected_url}")
    except Exception as e:
        logging.error(f"[Webhook健康检查] 检查失败: {e}")
...
def main():
    ...
    global bishu_5_price, bishu_15_price, bishu_50_price, bishu_100_price
    config = read_config('config.txt')
    TOKEN = get_env_first(['BOT_TOKEN'], config.get('TOKEN', ''))
    CUSTOMER_SERVICE_ID = get_env_first(['CUSTOMER_SERVICE_ID'], config.get('CUSTOMER_SERVICE_ID', ''))
    bot_id = get_env_first(['BOT_ID'], config.get('bot_id', ''))
    group_link = get_env_first(['GROUP_LINK'], config.get('group_link', ''))
    control_address = get_env_first(['CONTROL_ADDRESS'], config.get('control_address', ''))
    # 优先从环境变量读取私钥，其次读取 config.txt
    env_private_key = get_env_first(['USDT2TRX_PRIVATE_KEY', 'BOT_PRIVATE_KEY'])
    if env_private_key:
        privateKey = env_private_key
        usdt2trx_private_key = env_private_key
        ...
        logging.warning("警告：未配置USDT转TRX出款私钥，出币功能将被禁用")
    username = get_env_first(['BOT_API_USERNAME'], config.get('username', ''))
    password = get_env_first(['BOT_API_PASSWORD'], config.get('password', ''))
    ...
    # 唯一桥链：优先读取 .env 的 ENERGY_POOL_API_URL，fallback 到 config.txt
    env_energy_pool_api = get_env_energy_pool_api()
    if env_energy_pool_api:
        energy_pool_api = env_energy_pool_api
    elif 'energy_pool_api' not in config or not config['energy_pool_api'].strip():
    ...
    # Webhook 健康检查（每5分钟检查一次）
    if BOT_WEBHOOK_URL:
        scheduler.add_job(webhook_health_check, 'interval', minutes=5)
        logging.info("[Webhook健康检查] 已启用，每5分钟检查一次")
    scheduler.start()
    # 启动方式：优先使用 Webhook（需配置 BOT_WEBHOOK_URL），否则使用长轮询
    webhook_path = BOT_WEBHOOK_PATH or TOKEN
    if BOT_WEBHOOK_URL:
        # 先删除旧的 webhook 配置，避免 Telegram 缓存了旧的地址
        try:
            bot.delete_webhook()
            logging.info("已清除旧的 webhook 配置")
        except Exception as e:
            logging.warning(f"清除 webhook 配置失败（可能无旧配置）: {e}")
        
        listen_host = "0.0.0.0"
        webhook_full_url = build_public_endpoint(BOT_WEBHOOK_URL, webhook_path)
        # 仅当 webhook_full_url 有效时才设置 webhook（Telegram 不接受 0.0.0.0 等无效地址）
        if webhook_full_url and not webhook_full_url.startswith(('0.0.0.0', '127.0.0.1', 'localhost')):
            ...
            # 将充值通知路由挂载到同一个 tornado server
            try:
                import tornado.web
                import time as _time
                # 等待 httpd 初始化完成（start_webhook 在后台线程启动）
                for _ in range(20):
                    if updater.httpd and updater.httpd.http_server:
                        break
                    _time.sleep(0.2)
                # 通过 http_server.request_callback 拿到 WebhookAppClass（tornado.web.Application）
                tornado_app = updater.httpd.http_server.request_callback
                class TornadoRechargeHandler(tornado.web.RequestHandler):
                    def post(self):
                        import json, threading
                        try:
                            data = json.loads(self.request.body.decode('utf-8') or '{}')
                            self.set_header('Content-Type', 'application/json')
                            self.write(json.dumps({'success': True, 'message': 'accepted'}))
                            self.finish()
                ...
                class TornadoBishuHandler(tornado.web.RequestHandler):
                    def post(self):
                        import json
                        try:
                            data = json.loads(self.request.body.decode('utf-8') or '{}')
                            ...
                            if bot:
                                ...
                                if len(parts) >= 3:
                                    chat_id = int(parts[2])
                                    process_bishu_payment_success(chat_id, order_number, tx_hash, bot)
                            self.set_header('Content-Type', 'application/json')
                            self.write(json.dumps({'success': True, 'message': '通知已处理'}))
                        except Exception as e:
                            self.set_status(500)
                            self.write(json.dumps({'success': False, 'error': str(e)}))
                tornado_app.add_handlers(r'.*', [
                    (r'/api/recharge-notify', TornadoRechargeHandler),
                    (r'/api/bishu-payment-notify', TornadoBishuHandler),
                ])
                logging.info("充值通知路由已挂载到 webhook server (8443)")
            except Exception as te:
                logging.warning(f"挂载 tornado 路由失败: {te}", exc_info=True)
        else:
            logging.warning(f"Webhook URL 无效，跳过设置: {webhook_full_url}")
    else:
...
def start_notify_server():
    """启动HTTP服务器来接收能量池系统的通知（仅在非webhook模式下使用独立端口）"""
    try:
        # webhook模式下通知路由已挂载到8443，无需独立服务器
        if BOT_WEBHOOK_URL:
            logging.info("Webhook模式：充值通知路由已挂载到webhook server，跳过独立通知服务器")
            return
        notify_port = int(os.getenv('NOTIFY_SERVER_PORT', '8080'))
